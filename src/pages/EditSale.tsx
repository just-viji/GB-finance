...
const EditSale = () => {
  ...
  useEffect(() => {
    const fetchSale = async () => {
      ...
      if (error) {
        ...
        showError("Failed to fetch sale details: " + error.message);
        navigate('/reports'); // Redirect if sale not found or error
      } else if (data) {
        form.reset({
    ...
    if (user && id) {
      ...
      showError("You must be logged in to edit a sale.");
      navigate('/login');
    }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    if (!user || !id) {
      showError("Authentication error or sale ID missing.");
      return;
    }
      ...
      <Card className="w-full max-w-md mt-8">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <CardTitle className="text-2xl font-bold text-green-700 dark:text-green-400">Edit Sale</CardTitle>
