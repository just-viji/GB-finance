...
const EditSale = () => {
  ...
  useEffect(() => {
    const fetchSale = async () => {
      if (!user || !id) return;
      const { data, error } = await supabase.from('sales').select('*').eq('id', id).eq('user_id', user.id).single();
      if (error) {
        showError("Failed to fetch sale details.");
        navigate('/reports');
      } else if (data) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    if (!user || !id) return;
    const { error } = await supabase.from('sales').update({ ...values, date: format(values.date, 'yyyy-MM-dd') }).eq('id', id);
    if (error) {
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        <div className="flex items-center gap-4">
          ...
          <CardTitle className="text-2xl font-bold text-primary">Edit Sale</CardTitle>
        </div>
      </CardHeader>
      <CardContent>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" className="w-full">Update Sale</Button>
        </form>
      </CardContent>
    </Card>
  );
